...
system_prompt = """
Your task is to convert Python code into high performance Java 17 code.
...
def build_compile_command(file):
    # Compile Java source into the `output` directory (class files go to output/)
    output_dir = "output"
    src = os.path.join(output_dir, f"{file}.java")
    compile_command = [
        "javac",
        "-d",
        output_dir,
...
def build_run_command(file):
    # `file` is the Java class name to run; classes are in the `output` directory
    run_command = [
        "java",
        "-cp",
        "output",
...
def user_prompt_for(python):
    return f"""
Port this Python code to Java 17 with the fastest possible implementation that produces identical output in the least time.
...
Respond only with Java code. No explanation other than occasional comments.
Python code to port:
...
def write_output(model, cpp):
    ...
    # For Java porting we write to Main.java (the prompt asks for Main.java)
    file_path = os.path.join(output_dir, f"{safe_model}.java")
...
def port(model, python):
    try:
        ...
        # remove code fences for Java responses
...
def compile_and_run(model="main"):
    """Compile and run the generated Java file from the `output` directory.

    model: base filename (without extension) stored under `output/<model>.java`.
    """
    try:
...
with gr.Blocks() as ui:
    with gr.Row():
        ...
        cpp = gr.Textbox(label="Java code:", lines=28)
    with gr.Row():
